...
export function createAgentBranch(
  ...
  try {
    ...
    throw new Error(`git checkout -b failed: ${err.stderr?.toString() ?? err.message}`);
  }
...
export function createPullRequest(
  ...
  try {
    execSync(
      `gh pr create --head "${branch}" --title "${title.replace(/"/g, '\\"')}" --body "Auto-generated by Nova Orbit"`,
      { cwd: workdir, stdio: "pipe", timeout: 30000 },
    );
    log.info(`PR created for branch: ${branch}`);
  } catch (err: any) {
    // Fail silently — gh CLI may not be available or repo may not be on GitHub
    log.warn(`gh pr create skipped: ${err.stderr?.toString() ?? err.message}`);
  }
}
